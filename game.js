function gameStart() {
  ...
  function HighScore() {
    ctx.font = "20px Berlin Sans FB";
    ctx.fillStyle = "#ffffff";
    ctx.fillText("High Scorer: " + (localStorage.getItem("username") == null ? ' ' : localStorage.getItem("username")) + "\t \t \t \t \t \t \t" + "High Score: " + (localStorage.getItem("highScore") == null ? 0 : localStorage.getItem("highScore")), canvas.width - 450, 20);
  }
    ...
    if (gamelevel > 0) {
      ...
      if (y + dy < ballRadius)
        ...
        if (x >= paddleX - 5 && x <= paddleX + paddleWidth + 5) {
          hit.play();   
        if(gamelevel==2 && (dy>-6.5 && dy<6.5))
          dy = -dy-1;
      else if (gamelevel==3 && (dy>-7.5 && dy<7.5))
        dy = -dy-1;
      else if (gamelevel==4 && (dy>-8.5 && dy<8.5))
        dy = -dy-1;
      else if (gamelevel==5 && (dy>-9.5 && dy<9.5))
        dy = -dy-2;
      else dy=-dy ;   
